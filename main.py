...
class Slider:
    ...
    def update_rect(self, x, y, width, height):
        ...
        scale_y = HEIGHT / BASE_HEIGHT
        self.rect = pygame.Rect(x * scale_x, y * scale_y, width * scale_x, height * scale_y)
        self.handle_rect = pygame.Rect(self.rect.x + (self.value / self.max_val) * self.rect.width - 10 * scale_x, self.rect.y - 10 * scale_y, 20 * scale_x, 40 * scale_y)

    def draw(self, surface):
        scale_x = WIDTH / BASE_WIDTH
        scale_y = HEIGHT / BASE_HEIGHT
        label_surface = font.render(self.label, True, WHITE)
        surface.blit(label_surface, (self.rect.x - 350 * scale_x, self.rect.y - 10 * scale_y))
        pygame.draw.rect(surface, WHITE, self.rect, int(2 * scale_x))
        pygame.draw.rect(surface, GRAY, self.handle_rect)
        text_surface = font.render(f"{int(self.value)}%", True, WHITE)
        surface.blit(text_surface, (self.rect.x + self.rect.width + 20 * scale_x, self.rect.y - 10 * scale_y))

    def update(self, mouse_pos, mouse_pressed):
        scale_x = WIDTH / BASE_WIDTH
        if mouse_pressed and self.rect.collidepoint(mouse_pos):
            self.handle_rect.x = max(self.rect.x, min(mouse_pos[0] - 10 * scale_x, self.rect.x + self.rect.width - 20 * scale_x))
            if self.handle_rect.x <= self.rect.x:
                self.value = self.min_val
            elif self.handle_rect.x >= self.rect.x + self.rect.width - 20 * scale_x:
                self.value = self.max_val
            else:
                position_ratio = (self.handle_rect.x - self.rect.x) / (self.rect.width - 20 * scale_x)
...
class Option:
    ...
    def update_rect(self, x, y, width):
        ...
        self.left_arrow_rect = pygame.Rect(self.x - 100 * scale_x, self.y, 50 * scale_x, 50 * scale_y)
        self.right_arrow_rect = pygame.Rect(self.x + self.width + 50 * scale_x, self.y, 50 * scale_x, 50 * scale_y)

    def draw(self, surface):
        scale_x = WIDTH / BASE_WIDTH
        scale_y = HEIGHT / BASE_HEIGHT
        label_surface = font.render(self.label, True, WHITE)
        surface.blit(label_surface, (self.x - 400 * scale_x, self.y + 5 * scale_y))
        value_surface = font.render(str(self.values[self.current_index]), True, WHITE)
        surface.blit(value_surface, (self.x, self.y + 5 * scale_y))
        surface.blit(font.render("<", True, WHITE), (self.left_arrow_rect.x, self.left_arrow_rect.y + 5 * scale_y))
        surface.blit(font.render(">", True, WHITE), (self.right_arrow_rect.x, self.right_arrow_rect.y + 5 * scale_y))

    def update(self, mouse_pos, mouse_pressed):
        if mouse_pressed:
            if self.left_arrow_rect.collidepoint(mouse_pos) and self.current_index > 0:
                self.current_index -= 1
                self.apply()
            elif self.right_arrow_rect.collidepoint(mouse_pos) and self.current_index < len(self.values) - 1:
                self.current_index += 1
                self.apply()

    def apply(self):
        global WIDTH, HEIGHT, screen, background, main_menu_buttons, settings_menu_elements, font, player, platforms
        if self.label == "Разрешение":
            res = self.values[self.current_index].split("x")
            ...
            if new_width != WIDTH or new_height != HEIGHT:
                ...
                main_menu_buttons.extend([play_button, settings_button, exit_button])
                ...
                settings_menu_elements.clear()
                settings_menu_elements.extend([volume_slider_new, resolution_option_new, back_button_new])
                player = Player(BASE_WIDTH // 2 - 25, BASE_HEIGHT - 200, 50, 50)
                platforms = [
                    Platform(0, BASE_HEIGHT - 50, BASE_WIDTH, 50),
                    Platform(BASE_WIDTH // 2 - 150, BASE_HEIGHT - 200, 300, 20),
                    Platform(BASE_WIDTH // 4 - 100, BASE_HEIGHT - 350, 200, 20),
                    Platform(3 * BASE_WIDTH // 4 - 100, BASE_HEIGHT - 350, 200, 20),
                ]
