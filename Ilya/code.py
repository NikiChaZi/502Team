...
# Функция для масштабирования фонового изображения с сохранением соотношения сторон
def scale_background(image, target_width, target_height):
    if image is None:
        return None, (0, 0)  # Возвращаем None и нулевые координаты, если изображение не загружено
    # Получаем размеры исходного изображения
    img_width, img_height = image.get_size()
    # Вычисляем соотношение сторон изображения и окна
    img_ratio = img_width / img_height
    screen_ratio = target_width / target_height

    # Определяем размеры для масштабирования с сохранением соотношения сторон
    if img_ratio > screen_ratio:
        # Изображение шире, чем окно - масштабируем по высоте
        new_height = target_height
        new_width = int(new_height * img_ratio)
    else:
        # Изображение выше, чем окно - масштабируем по ширине
    ...
    # Центрируем изображение, вычисляя смещение по осям X и Y
...
try:
    ...
    background, background_pos = scale_background(background, WIDTH, HEIGHT)  # Масштабируем и центрируем фон
except:
    background = None  # Если загрузка не удалась, фон будет None
    background_pos = (0, 0)  # Нулевые координаты для отображения фона
...
class Button:
    ...
    def update_rect(self, x, y, width, height):
        ...
        self.rect = pygame.Rect(x * scale_x, y * scale_y, width * scale_x,
                                height * scale_y)  # Масштабируем прямоугольник кнопки
...
class Slider:
    ...
    def update_rect(self, x, y, width, height):
        ...
        scale_y = HEIGHT / BASE_HEIGHT  # Масштаб по оси Y
        self.rect = pygame.Rect(x * scale_x, y * scale_y, width * scale_x,
                                height * scale_y)  # Масштабируем прямоугольник ползунка
        self.handle_rect = pygame.Rect(self.rect.x + (self.value / self.max_val) * self.rect.width - 10 * scale_x,
                                       self.rect.y - 10 * scale_y, 20 * scale_x,
                                       40 * scale_y)  # Масштабируем ручку ползунка
    ...
    def draw(self, surface):
        ...
        self.rect.x - 350 * scale_x, self.rect.y - 10 * scale_y))  # Отрисовываем название слева от ползунка
        ...
        text_surface = font.render(f"{int(self.value)}%", True,
                                   WHITE)  # Создаем поверхность с текущим значением громкости
        surface.blit(text_surface, (self.rect.x + self.rect.width + 20 * scale_x,
                                    self.rect.y - 10 * scale_y))  # Отрисовываем значение справа от ползунка
    ...
    def update(self, mouse_pos, mouse_pressed):
        ...
        if mouse_pressed and self.rect.collidepoint(mouse_pos):  # Если мышь нажата и курсор над ползунком
            self.handle_rect.x = max(self.rect.x, min(mouse_pos[0] - 10 * scale_x,
                                                      self.rect.x + self.rect.width - 20 * scale_x))  # Ограничиваем движение ручки ползунка
            if self.handle_rect.x <= self.rect.x:  # Если ручка в крайнем левом положении
                ...
                position_ratio = (self.handle_rect.x - self.rect.x) / (
                            self.rect.width - 20 * scale_x)  # Вычисляем пропорцию положения ручки
                self.value = self.min_val + position_ratio * (
                            self.max_val - self.min_val)  # Вычисляем новое значение громкости
...
class Option:
    ...
    def update_rect(self, x, y, width):
        ...
        self.left_arrow_rect = pygame.Rect(self.x - 100 * scale_x, self.y, 50 * scale_x,
                                           50 * scale_y)  # Масштабируем область левой стрелки
        self.right_arrow_rect = pygame.Rect(self.x + self.width + 50 * scale_x, self.y, 50 * scale_x,
                                            50 * scale_y)  # Масштабируем область правой стрелки
    ...
    def draw(self, surface):
        ...
        label_surface = font.render(self.label, True, WHITE)  # Создаем поверхность с названием настройки
        surface.blit(label_surface,
                     (self.x - 400 * scale_x, self.y + 5 * scale_y))  # Отрисовываем название слева от значения
        value_surface = font.render(str(self.values[self.current_index]), True,
                                    WHITE)  # Создаем поверхность с текущим значением
        surface.blit(value_surface, (self.x, self.y + 5 * scale_y))  # Отрисовываем значение
        surface.blit(font.render("<", True, WHITE),
                     (self.left_arrow_rect.x, self.left_arrow_rect.y + 5 * scale_y))  # Отрисовываем левую стрелку
        surface.blit(font.render(">", True, WHITE),
                     (self.right_arrow_rect.x, self.right_arrow_rect.y + 5 * scale_y))  # Отрисовываем правую стрелку

    # Метод для обновления состояния элемента (переключение значений)
    def update(self, mouse_pos, mouse_pressed):
        if mouse_pressed:  # Если мышь нажата
            if self.left_arrow_rect.collidepoint(
                    mouse_pos) and self.current_index > 0:  # Если нажата левая стрелка и есть предыдущее значение
                self.current_index -= 1  # Уменьшаем индекс значения
                self.apply()  # Применяем новое значение
            elif self.right_arrow_rect.collidepoint(mouse_pos) and self.current_index < len(
                    self.values) - 1:  # Если нажата правая стрелка и есть следующее значение
                self.current_index += 1  # Увеличиваем индекс значения
                self.apply()  # Применяем новое значение

    # Метод для применения выбранного значения
    def apply(self):
        global WIDTH, HEIGHT, screen, background, background_pos, main_menu_buttons, settings_menu_elements, font
        if self.label == "Разрешение":  # Если настройка - это разрешение
            ...
            if new_width != WIDTH or new_height != HEIGHT:  # Если новое разрешение отличается от текущего
                ...
                if background:
                    ...
                    background, background_pos = scale_background(pygame.image.load('background.png'), WIDTH, HEIGHT)
                ...
                play_button = Button("Играть", BASE_WIDTH // 2 - button_width // 2,
                                     BASE_HEIGHT // 2 - button_height - button_spacing, button_width, button_height)
                settings_button = Button("Настройки", BASE_WIDTH // 2 - button_width // 2, BASE_HEIGHT // 2,
                                         button_width, button_height)
                exit_button = Button("Выход", BASE_WIDTH // 2 - button_width // 2,
                                     BASE_HEIGHT // 2 + button_height + button_spacing, button_width, button_height)
                ...
                volume_slider_new = Slider("Громкость", BASE_WIDTH // 2 + 0, BASE_HEIGHT // 2 - 80, 300, 20, 0, 100,
                                           settings_menu_elements[0].value)
                resolution_option_new = Option("Разрешение", BASE_WIDTH // 2 + 50, BASE_HEIGHT // 2, 300, resolutions,
                                               self.current_index)
                back_button_new = Button("Назад", BASE_WIDTH // 2 - button_width // 2, BASE_HEIGHT // 2 + 160,
                                         button_width, button_height)
                settings_menu_elements.clear()  # Очищаем старый список элементов настроек
                settings_menu_elements.extend(
                    [volume_slider_new, resolution_option_new, back_button_new])  # Добавляем новые элементы
...
play_button = Button("Играть", BASE_WIDTH // 2 - button_width // 2, BASE_HEIGHT // 2 - button_height - button_spacing,
                     button_width, button_height)
settings_button = Button("Настройки", BASE_WIDTH // 2 - button_width // 2, BASE_HEIGHT // 2, button_width,
                         button_height)
exit_button = Button("Выход", BASE_WIDTH // 2 - button_width // 2, BASE_HEIGHT // 2 + button_height + button_spacing,
                     button_width, button_height)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if current_menu == "main":  # Если текущее меню - главное
            if event.type == pygame.MOUSEBUTTONDOWN:  # Если нажата кнопка мыши
                if play_button.is_hovered(mouse_pos):  # Если нажата кнопка "Играть"
                    print("Запуск игры!")  # Выводим сообщение (заглушка для запуска игры)
                elif settings_button.is_hovered(mouse_pos):  # Если нажата кнопка "Настройки"
                    current_menu = "settings"  # Переходим в меню настроек
                elif exit_button.is_hovered(mouse_pos):  # Если нажата кнопка "Выход"
                    running = False  # Завершаем цикл

        elif current_menu == "settings":  # Если текущее меню - настройки
            if event.type == pygame.MOUSEBUTTONDOWN:  # Если нажата кнопка мыши
                if back_button.is_hovered(mouse_pos):  # Если нажата кнопка "Назад"
    ...
    # Отрисовка фона
    if background:
        screen.blit(background, background_pos)  # Отрисовываем фон с учетом смещения
    else:
